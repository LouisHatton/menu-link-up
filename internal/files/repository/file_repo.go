...
var selectAll string = "SELECT `id`, `user_id`, `name`, `updated_at`, `created_at`, `slug`, `s3_bucket`, `s3_key` FROM `files` "
...
func (r *FileRepo) Create(ctx context.Context, file *files.File) error {
	query := "INSERT INTO `files` (`id`, `user_id`, `name`, `updated_at`, `created_at`, `slug`, `s3_bucket`, `s3_key`) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
...
func (r *FileRepo) Update(ctx context.Context, file *files.File) error {
	query := "UPDATE `files` SET `user_id` = ?, `name` = ?, `updated_at` = ?, `created_at` = ?, `slug` = ?, `s3_bucket` = ?, `s3_key` = ? WHERE `id` = ?"
